<template>
    ...
    <!-- Main modal -->
    <Transition>
        <div id="edit-user-modal" v-if="open"
...
Vue.watch(props, (newValue) => {
    form.value = DeepCopy(newValue.user);
});
...
async function request() {
    try {
        const response = await axios.patch(import.meta.env.VITE_API_URL + "/users/" + props.user.id, form);
